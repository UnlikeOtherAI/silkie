...
test.describe("System tab", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector("#panel-system:not(.hidden)", { timeout: 5000 });
  });

  test("health checks show ok", async ({ page }) => {
    await expect(page.locator("#health-healthz")).toContainText("ok", { timeout: 5000 });
    await expect(page.locator("#health-readyz")).toContainText("ok", { timeout: 5000 });
  });

  test("server info displays version", async ({ page }) => {
    await expect(page.locator("#sys-version")).toContainText("0.1.0", { timeout: 5000 });
  });

  test("infrastructure status is visible", async ({ page }) => {
    await expect(page.locator("#sys-opa")).toContainText("allow-all", { timeout: 5000 });
  });
  ...
  test("audit log section is visible", async ({ page }) => {
    ...
    // Either we see events, empty state, or error — any is fine as long as the section loaded.
